...
contract RiskOracleTest is Test {
    ...
    function test_VerifyRiskPayload_ValidSignature() public {
        IRiskOracle.RiskPayload memory payload = _makePayload(0, 1);
        ...
        assertTrue(oracle.isNonceUsed(user, 1));
    }
    ...
    function test_VerifyRiskPayload_ExpiredTimestamp_Reverts() public {
        ...
        IRiskOracle.RiskPayload memory payload = _makePayload(oldTimestamp, 1);
        bytes memory sig = _signPayload(payload, ORACLE_PRIVATE_KEY);
    ...
    function test_VerifyRiskPayload_AtValidityBoundary_Succeeds() public {
        ...
        IRiskOracle.RiskPayload memory payload = _makePayload(boundaryTimestamp, 1);
        bytes memory sig = _signPayload(payload, ORACLE_PRIVATE_KEY);
    ...
    function test_VerifyRiskPayload_JustPastValidity_Reverts() public {
        ...
        IRiskOracle.RiskPayload memory payload = _makePayload(pastTimestamp, 1);
        bytes memory sig = _signPayload(payload, ORACLE_PRIVATE_KEY);
    ...
    function test_VerifyRiskPayload_DifferentNonce_SameUser_Succeeds() public {
        IRiskOracle.RiskPayload memory payload1 = _makePayload(0, 1);
        IRiskOracle.RiskPayload memory payload2 = _makePayload(0, 2);
    ...
    function test_GetPayloadDigest_ChangesWithPayload() public view {
        IRiskOracle.RiskPayload memory p1 = _makePayload(0, 1);
        IRiskOracle.RiskPayload memory p2 = _makePayload(0, 2);
