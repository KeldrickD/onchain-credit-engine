...
library SignatureVerifier {
    ...
    bytes32 internal constant RISK_PAYLOAD_V2_BY_KEY_TYPEHASH =
        keccak256(
            "RiskPayloadV2ByKey(bytes32 subjectKey,uint16 score,uint8 riskTier,uint16 confidenceBps,bytes32 modelId,bytes32 reasonsHash,bytes32 evidenceHash,uint64 timestamp,uint64 nonce)"
        );
    ...
    function hashRiskPayloadV2ByKey(
        bytes32 subjectKey,
        uint16 score,
        uint8 riskTier,
        uint16 confidenceBps,
        ...
        return
            keccak256(
                abi.encode(
                    RISK_PAYLOAD_V2_BY_KEY_TYPEHASH,
                    subjectKey,
                    score,
                    riskTier,
                    confidenceBps,
